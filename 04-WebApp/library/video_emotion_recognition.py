...
def gen():
    ...
    while end - start < max_time :
        ...
        # Image to gray scale
        # frame = cv2.imread('04-WebApp/tmp/t.jpg')
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        ...
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n'+ open('04-WebApp/tmp/t.jpg', 'rb').read() + b'\r\n')
        ...
        if end-start > max_time - 1 :
            with open("04-WebApp/static/js/db/histo_perso.txt", "w") as d:
                d.write("density"+'\n')
                for val in predictions :
                    d.write(str(val)+'\n')
                    
                
            with open("04-WebApp/static/js/db/histo.txt", "a") as d:
                for val in predictions :
                    d.write(str(val)+'\n')
            ...
            import csv
            with open("04-WebApp/static/js/db/prob.csv", "w") as d:
                writer = csv.writer(d)
                for row in rows:
                    writer.writerow(row)
         

            with open("04-WebApp/static/js/db/prob_tot.csv", "a") as d:
                writer = csv.writer(d)
                for row in rows:
                    writer.writerow(row)
...
# y_pred = faceCascade.predict(x_test)
